...
Для корректного `server_name` внутри nginx используйте переменную окружения `SERVER_NAME`.
- Локально значение по умолчанию — `localhost`.
- Для боевого домена запустите, например:

```bash
SERVER_NAME=iotgreenhouse.ru docker compose up --build -d
```
